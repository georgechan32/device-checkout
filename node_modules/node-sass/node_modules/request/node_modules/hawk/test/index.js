...
describe('Hawk', function () {
    ...
    it('generates a header then successfully parse it (configuration)', function (done) {
        ...
        credentialsFunc('123456', function (err, credentials1) {
            ...
            req.authorization = Hawk.client.header(Url.parse('http://example.com:9030/resource/4?filter=a'), req.method, { credentials: credentials1, ext: 'some-app-data' }).field;
    ...
    it('generates a header then successfully parse it (node request)', function (done) {
        ...
        credentialsFunc('123456', function (err, credentials1) {
            ...
            var reqHeader = Hawk.client.header('http://example.com:9030/resource/4?filter=a', req.method, { credentials: credentials1, ext: 'some-app-data', payload: payload, contentType: req.headers['content-type'] });
    ...
    it('generates a header then successfully parse it (absolute request uri)', function (done) {
        ...
        var req = {
            method: 'POST',
            url: 'http://example.com:9030/resource/4?filter=a',
            headers: {
                host: 'example.com:9030',
                'content-type': 'text/plain;x=y'
            }
    ...
    it('generates a header then successfully parse it (with hash)', function (done) {
        ...
        credentialsFunc('123456', function (err, credentials1) {
            ...
            req.authorization = Hawk.client.header('http://example.com:9030/resource/4?filter=a', req.method, { credentials: credentials1, payload: 'hola!', ext: 'some-app-data' }).field;
            Hawk.server.authenticate(req, credentialsFunc, {}, function (err, credentials2, artifacts) {
    ...
    it('generates a header then successfully parse it (app)', function (done) {
        ...
        credentialsFunc('123456', function (err, credentials1) {
            ...
            req.authorization = Hawk.client.header('http://example.com:9030/resource/4?filter=a', req.method, { credentials: credentials1, ext: 'some-app-data', app: 'asd23ased' }).field;
            Hawk.server.authenticate(req, credentialsFunc, {}, function (err, credentials2, artifacts) {
    ...
    it('generates a header then successfully parse it (app, dlg)', function (done) {
        ...
        credentialsFunc('123456', function (err, credentials1) {
            ...
            req.authorization = Hawk.client.header('http://example.com:9030/resource/4?filter=a', req.method, { credentials: credentials1, ext: 'some-app-data', app: 'asd23ased', dlg: '23434szr3q4d' }).field;
            Hawk.server.authenticate(req, credentialsFunc, {}, function (err, credentials2, artifacts) {
    ...
    it('generates a header for one resource then fail to authenticate another', function (done) {
        ...
        credentialsFunc('123456', function (err, credentials1) {
            ...
            req.authorization = Hawk.client.header('http://example.com:9030/resource/4?filter=a', req.method, { credentials: credentials1, ext: 'some-app-data' }).field;
