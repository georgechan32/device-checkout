...
var options = { port: 9030 };

var config = Hoek.applyToDefaults(defaults, options); // results in { host: "localhost", port: 9030 }
```
...
var defaults = { host: "localhost", port: 8000 };
var options = { host: null, port: 9030 };

var config = Hoek.applyToDefaults(defaults, options, true); // results in { host: null, port: 9030 }
```
...
var options = { server: { port: 9030 } };

var config = Hoek.applyToDefaultsWithShallow(defaults, options, ['server']); // results in { server: { port: 9030 }, name: 'example' }
```
